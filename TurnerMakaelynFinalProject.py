...
class EcoFriendlyBudgetTracker:
    ...
    def load_images(self):
        try:
            self.leaf_image = ImageTk.PhotoImage(Image.open("leaf.png").resize((100, 100)))
            self.sun_image = ImageTk.PhotoImage(Image.open("sun.jpeg").resize((100, 100)))
    ...
    def main_dashboard(self):
        ...
        if self.leaf_image:
            tk.Label(self.root, image=self.leaf_image).pack(pady=10)
    ...
    def add_expense(self):
        ...
        if self.sun_image:
            tk.Label(self.root, image=self.sun_image).pack(pady=10)
